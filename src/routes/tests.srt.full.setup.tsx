...
function SetupPage() {
  ...
  async function commence() {
    ...
    if (user && ADMIN_EMAILS.includes(user.email ?? "")) {
      ...
      sessionStorage.setItem("srt_word_count", "60");
      navigate({ to: "/tests/srt/full/test" });
      return;
    }
    ...
    if (!user) {
      ...
      const attemptId = await recordTestAttempt(undefined, anonId, "srt_full");
      if (attemptId) sessionStorage.setItem("forgessb_current_attempt_id", attemptId);
    } else {
    ...
    <section className="relative">
      ...
      <div className="relative mx-auto max-w-2xl px-6 py-20">
        ...
        <p className="mt-8 text-base leading-relaxed text-foreground/80">
          This full simulation mirrors the standard SSB SRT format. The run is fixed at
          sixty situations with thirty minutes total duration.
        </p>
        ...
        <div className="mt-10 border border-gold/40 bg-surface-1/50 p-6 sm:p-8">
          ...
          <p className="mt-2 font-serif text-3xl text-gold sm:text-4xl">60 situations · 30 min</p>
          <p className="mt-4 text-sm leading-relaxed text-foreground/75">
            Once commenced, situations will advance every 30 seconds. Complete all responses in
        ...
        <div className="mt-12 flex flex-col items-center gap-4 border-t border-border pt-10">
          ...
          <button
            type="button"
            onClick={() => navigate({ to: "/tests/srt/full/instructions" })}
